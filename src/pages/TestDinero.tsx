...
import {
  ArrowLeft, ArrowRight, Sparkles, Mail, Phone, Loader2, MessageCircle,
  Compass, Send, Check,
} from 'lucide-react';
...
const TestDinero: React.FC = () => {
  ...
  const progress = step === 'quiz' ? Math.round((qIndex / PREGUNTAS_DINERO.length) * 100) : 0;
  ...
  const pickAnswer = (patronKey: string) => {
  ...
  const submitAndReveal = async () => {
    setSubmitting(true);
    try {
      await fetch('/api/cursos?action=dinero-submit', {
  ...
  return (
    <div className="min-h-screen bg-brand-beige flex flex-col">
      ...
      {step === 'quiz' && (
        <div className="h-1 bg-gray-200"><div className="h-1 bg-brand-gold transition-all duration-300" style={{ width: `${progress}%` }} /></div>
      )}
        ...
        <div className="w-full max-w-lg">
          ...
          {step === 'intro' && (
            <div className="text-center">
              ...
              <button onClick={() => setStep('quiz')}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 gold-gradient text-white font-bold py-4 px-10 rounded-full shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all text-base">
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {PREGUNTAS_DINERO[qIndex].opciones.map((op, i) => (
                  <button key={i} onClick={() => pickAnswer(op.patron)}
                    className="w-full text-left bg-white hover:bg-brand-gold/5 border-2 border-gray-200 hover:border-brand-gold rounded-2xl px-5 py-4 transition-all active:scale-[0.98] touch-manipulation">
                    <span className="text-brand-dark text-sm sm:text-base">{op.label}</span>
                  </button>
                ))}
              </div>
          ...
          {step === 'capture' && (
            <div className="bg-white rounded-3xl shadow-sm p-6 sm:p-8 text-center">
              ...
              <div className="space-y-3 text-left mb-2">
                ...
                <div className="relative">
                  <Phone className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 opacity-0" />
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="Tu nombre (opcional)"
                    className="w-full border-2 border-gray-200 rounded-xl pl-4 pr-4 py-3.5 focus:outline-none focus:border-brand-gold text-brand-dark" />
                </div>
              </div>

              <button onClick={submitAndReveal} disabled={submitting}
                className="w-full mt-4 flex items-center justify-center gap-2 gold-gradient text-white font-bold py-4 rounded-xl shadow-md disabled:opacity-70 min-h-[56px] text-base">
                {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Ver mi resultado <ArrowRight className="w-5 h-5" /></>}
              </button>
              <button onClick={submitAndReveal} disabled={submitting} className="text-xs text-gray-400 hover:text-gray-600 mt-3">
          ...
          {step === 'result' && patron && (
            <div>
              ...
              <div className="bg-white rounded-3xl shadow-sm p-6 sm:p-8 text-center mb-5 relative overflow-hidden">
              ...
              <div className="bg-brand-dark rounded-3xl p-6 sm:p-7 text-white text-center mb-4">
                ...
                <a href={waHref} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-2 gold-gradient text-white font-bold py-3.5 px-8 rounded-full shadow-md hover:shadow-lg transition-all">
                  <MessageCircle className="w-5 h-5" /> Quiero saber más
                </a>
              </div>

              {/* Newsletter */}
              <Link to="/recursos" className="block bg-white rounded-2xl p-5 border border-gray-100 hover:border-brand-gold/40 transition-all">
