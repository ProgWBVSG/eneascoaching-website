...
const TestDineroTab: React.FC<{ token: string }> = ({ token }) => {
  ...
  const copyDineroLink = async () => {
    try {
      ...
      prompt('Copiá el link manualmente:', dineroLink);
    }
  };
  ...
  return (
    <div>
      {/* Link general — el mismo link para todos, se manda desde ManyChat/WhatsApp */}
      <div className="bg-brand-dark rounded-2xl p-5 mb-5 text-white">
        <div className="flex items-center gap-2 mb-1"><Coins className="w-5 h-5 text-brand-gold" /><h3 className="font-heading font-bold">Link del Test de Dinero</h3></div>
        <p className="text-sm text-gray-300 mb-4">Es un solo link para todas las personas (no hace falta generar uno por cada una). Copiálo y mandalo por WhatsApp, ManyChat o donde quieras.</p>
        <div className="flex items-center justify-between gap-3 bg-white/10 rounded-xl p-3">
          <code className="text-brand-gold font-mono text-sm truncate">{dineroLink}</code>
          <button onClick={copyDineroLink}
...
const TestLiderazgoTab: React.FC<{ token: string }> = ({ token }) => {
  const [items, setItems] = useState<LiderazgoSubmission[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteConfirm, setDeleteConfirm] = useState<string | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [linkCopied, setLinkCopied] = useState(false);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/cursos?action=liderazgo-submissions', { headers: { 'x-admin-token': token } });
      const data = await res.json();
      setItems(Array.isArray(data) ? data : []);
    } finally { setLoading(false); }
  }, [token]);
  ...
  const del = async (id: string) => {
    await fetch(`/api/cursos?action=liderazgo-submission&id=${id}`, { method: 'DELETE', headers: { 'x-admin-token': token } });
    setDeleteConfirm(null); load();
  };
  ...
  return (
    <div>
      {/* Link general */}
      <div className="bg-brand-dark rounded-2xl p-5 mb-5 text-white">
        <div className="flex items-center gap-2 mb-1"><Compass className="w-5 h-5 text-brand-gold" /><h3 className="font-heading font-bold">Link del Test de Liderazgo</h3></div>
        <p className="text-sm text-gray-300 mb-4">Un solo link para todas las personas. Copiálo y mandalo por Instagram, WhatsApp o donde quieras.</p>
        <div className="flex items-center justify-between gap-3 bg-white/10 rounded-xl p-3">
      ...
      {!loading && items.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-lg">Todavía no hay respuestas del test de liderazgo.</p>
          <p className="text-gray-300 text-sm mt-1">Compartí el link de arriba para recibir leads.</p>
        </div>
      )}
...
// ── Tab de leads del Test de Reuniones (Reuniones con Ceci) ─────────
interface ReunionesSubmission {
  id: string; name: string | null; contact: string | null; profile: Area;
  answers: number[]; created_at: string;
}

const TestReunionesTab: React.FC<{ token: string }> = ({ token }) => {
  const [items, setItems] = useState<ReunionesSubmission[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteConfirm, setDeleteConfirm] = useState<string | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [linkCopied, setLinkCopied] = useState(false);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/cursos?action=reuniones-submissions', { headers: { 'x-admin-token': token } });
      const data = await res.json();
      setItems(Array.isArray(data) ? data : []);
    } finally { setLoading(false); }
  }, [token]);
  ...
  const del = async (id: string) => {
    await fetch(`/api/cursos?action=reuniones-submission&id=${id}`, { method: 'DELETE', headers: { 'x-admin-token': token } });
    setDeleteConfirm(null); load();
  };
  ...
  const counts: Record<string, number> = {};
  items.forEach(i => { counts[i.profile] = (counts[i.profile] || 0) + 1; });

  return (
    <div>
      <div className="bg-brand-dark rounded-2xl p-5 mb-5 text-white">
        <div className="flex items-center gap-2 mb-1"><Users className="w-5 h-5 text-brand-gold" /><h3 className="font-heading font-bold">Link del Test de Reuniones</h3></div>
      ...
      {!loading && items.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-lg">Todavía no hay respuestas del test de reuniones.</p>
          <p className="text-gray-300 text-sm mt-1">Compartí el link de arriba para recibir leads.</p>
        </div>
      )}
      ...
      <>
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-5">
        <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Limitación principal detectada ({items.length} respuestas)</p>
        <div className="flex flex-wrap gap-2">
          {Object.entries(counts).map(([key, count]) => {
            const p = PERFILES_REUNIONES[key as Area];
            if (!p) return null;
            return (
              <span key={key} className="text-xs font-semibold px-3 py-1.5 rounded-full" style={{ background: `${p.color}1a`, color: p.color }}>
                {p.titulo} · {count}
      ...
      <div className="space-y-2">
        {items.map(sub => {
          const p = PERFILES_REUNIONES[sub.profile];
          const isOpen = expandedId === sub.id;
          return (
            <div key={sub.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <button onClick={() => setExpandedId(isOpen ? null : sub.id)}
              ...
              {isOpen && (
                <div className="border-t border-gray-100 px-4 py-4 bg-gray-50">
                  ...
                  <div className="space-y-2 mb-4">
                    {PREGUNTAS_REUNIONES.map((preg, i) => {
